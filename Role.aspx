...
<head runat="server">
    ...
    <script type="text/javascript">
        function GridMenuItemClick(e) {
            ...
            if (name == "add") AddRole();
            if (name == "update") EditRole();
            if (name == "delete") DeleteRole();
        }
        function AddRole() {
            GridRole.AddNewRow();
        }
        ...
        function EditRole() {
            ...
            var valueRow = GridRole.GetFocusedRowIndex();
            GridRole.StartEditRow(valueRow);
        }
        function DeleteRole() {
            var valueRow = GridRole.GetFocusedRowIndex();
            GridRole.DeleteRow(valueRow);
        }

        function DeleteRole() {
            GridRole.GetRowValues(GridRole.GetFocusedRowIndex(), 'Nom_Role', SelectshowDeleteRole);

        }

        function SelectshowDeleteRole(value) {
...
<body>
    <form id="form1" runat="server">
        <dx:ASPxPopupMenu ID="ASPxPopupRole" runat="server" ClientInstanceName="ASPxPopupRole" CssPostfix="Glass" Theme="Glass">
            <Items>
    ...
    <dx:ASPxGridView ID="GridRole" ClientInstanceName="GridRole" Width="100%" runat="server" 
                AutoGenerateColumns="False" DataSourceID="SQLRole" KeyFieldName="ID_Role" 
                Theme="Aqua">
        <ClientSideEvents Init="function(s, e) { 
                      var popup = window.parent; 
                      popup.window['GridRole'] = GridRole;}"
                                                ContextMenu="function(s, e) {
                                 var x = ASPxClientUtils.GetEventX(e.htmlEvent);
                                 var y = ASPxClientUtils.GetEventY(e.htmlEvent);
   
                          if(e.objectType == 'row'){
                           GridRole.SetFocusedRowIndex(e.index);
                           ASPxPopupRole.ShowAtPos(x,y); 
                                                          }
                            }" />
                
              
              <Columns>
                    <dx:GridViewDataTextColumn FieldName="ID_Role"  Visible="false"  VisibleIndex="0">
                        <EditFormSettings Visible="False" />
                    </dx:GridViewDataTextColumn>
                    <dx:GridViewDataTextColumn FieldName="Nom_Role" Name="Role" VisibleIndex="1">
                    </dx:GridViewDataTextColumn>
                </Columns>
        ...
        <asp:SqlDataSource ID="SQLRole" runat="server" 
            ...
            <InsertParameters>
                <asp:Parameter Name="Nom_Role" Type="String" />
            </InsertParameters>
            <UpdateParameters>
                <asp:Parameter Name="ID_Role" Type="Int32" />
                <asp:Parameter Name="Nom_Role" Type="String" />
