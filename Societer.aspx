<%@ Page Language="C#" AutoEventWireup="true" CodeFile="Societer.aspx.cs" Inherits="Societer" %>
    ...
    <title></title>
     <script type="text/javascript">
        function GridMenuItemClick(e) {
            ...
            if (name == "add") AddSocieter();
            if (name == "update") EditSocieter();
            if (name == "delete") DeleteSocieter();
        }
        function AddSocieter() {
            
            GridSocieter.AddNewRow();
        }
        function EditSocieter() {
            var valueRow = GridSocieter.GetFocusedRowIndex();
            GridSocieter.StartEditRow(valueRow);
        }
        function DeleteSocieter() {
            var valueRow = GridSocieter.GetFocusedRowIndex();
            GridSocieter.DeleteRow(valueRow);
        }
        function DeleteSocieter() {
            GridSocieter.GetRowValues(GridSocieter.GetFocusedRowIndex(), 'Nom_Societe', SelectshowDeleteSocieter);

        }

        function SelectshowDeleteSocieter(value) {
...
<body>
    <form id="form1" runat="server">
        <dx:ASPxPopupMenu ID="ASPxPopupSocieter" runat="server" ClientInstanceName="ASPxPopupSocieter" CssPostfix="Glass" Theme="Glass">
            <Items>
    ...
    <div>
        ...
        <Items>
           ...
           <dx:ASPxGridView ID="GridSocieter" ClientInstanceName="GridSocieter" Width="100%" runat="server" 
                AutoGenerateColumns="False" DataSourceID="SqlSocieter" KeyFieldName="ID_Societe" 
                Theme="Aqua">
               <ClientSideEvents Init="function(s, e) { 
                      var popup = window.parent; 
                      popup.window['GridSocieter'] = GridSocieter;}"
                                                 ContextMenu="function(s, e) {
                                 var x = ASPxClientUtils.GetEventX(e.htmlEvent);
                                 var y = ASPxClientUtils.GetEventY(e.htmlEvent);
   
                          if(e.objectType == 'row'){
                           GridSocieter.SetFocusedRowIndex(e.index);
                           ASPxPopupSocieter.ShowAtPos(x,y); 
                                                          }
                            }" />
                <Columns>
                    <dx:GridViewDataTextColumn FieldName="ID_Societe" Visible="false" ReadOnly="True" VisibleIndex="0">
                        <EditFormSettings Visible="False" />
                    </dx:GridViewDataTextColumn>
                    <dx:GridViewDataTextColumn FieldName="Nom_Societe" Caption="Nom de societer" VisibleIndex="1">
                    </dx:GridViewDataTextColumn>
                </Columns>
               <SettingsBehavior AllowFocusedRow="true" />
            </dx:ASPxGridView>
        <asp:SqlDataSource ID="SqlSocieter" runat="server" ConnectionString="<%$ ConnectionStrings:EmployeConnectionString %>"
             SelectCommand="SelectSocieter" DeleteCommand="DeleteSocieter" DeleteCommandType="StoredProcedure" InsertCommand="InsertSocieter" InsertCommandType="StoredProcedure" SelectCommandType="StoredProcedure" UpdateCommand="UpdateSocieter" UpdateCommandType="StoredProcedure">
            <DeleteParameters>
                <asp:Parameter Name="ID_Societe" Type="Int32" />
            </DeleteParameters>
            <InsertParameters>
                <asp:Parameter Name="Nom_Societe" Type="String" />
            </InsertParameters>
            <UpdateParameters>
                <asp:Parameter Name="Nom_Societe" Type="String" />
                <asp:Parameter Name="ID_Societe" Type="Int32" />
